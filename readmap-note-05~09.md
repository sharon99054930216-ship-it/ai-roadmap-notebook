...
10.機器人為什麼還沒進你家  
Distribution Shift(數據分佈偏移):訓練集（如 DROID、BridgeData）裡的桌面，再亂也只包含特定幾種桌巾、光線與雜物組合。  
Long-Horizon(長時序任務的概率):缺乏自我糾錯機制。只要第 2 步掉落了物品或沒有對齊，後續的 8 步全部都會基於錯誤的當前狀態繼續執行，導致整套動作徹底崩潰。  
Contact-Rich(富接觸/微觀力學任務):視覺看不到力。插拔插頭、擰螺絲、蓋寶特瓶蓋這類任務，極度依賴手指末端的觸覺感測與扭力反饋。  
Deformable Objects(柔性物體與變形體):狀態空間（State Space）維度爆炸。剛體（如馬克杯）只需要 6-DoF 位姿就能完整描述；但衣服、繩子、麵糰、軟塑膠袋等軟性物體，其幾何變化是無窮維度的。    
Safety(安全機制):VLM 的自回歸本質缺乏「我不確定」的止損機制。大模型永遠會強制輸出下一個概率最高的 Token。   
Generalization(抽象泛化能力降維打擊):機器人無法像人一樣把「煮飯」這個抽象任務拆解成一連串可彈性調整的步驟。一旦中間有一個小步驟失敗，整條動作鏈就會崩潰，因為它缺乏在抽象層面進行「任務重排」的能力。
...
10.研究機會  
動態場景的幾何失效（Dynamic Scenes）：  
硬性假設：目前的 DUSt3R / VGGT 底層多半假設世界是靜態的。  
物理崩潰：當畫面中出現移動的人類、車輛或軟體物體時，模型會將其視為離群值或噪聲，導致點雲出現拉伸、重影或幾何對齊失敗。  
絕對尺度的模糊性（Scale Ambiguity）：  
缺乏真實單位：單眼 Pointmap 預測本質上是「比例正確，但無絕對長度」，模型分不出畫面裡的是玩具模型還是真實汽車。  
未完待續：雖然 VGGT 透過多圖全局對齊顯著改善了相對尺度的一致性，但要做到不需要 Sensor 就能精準輸出 1:1 的公制單位，依然相當困難。  
長影片與串流運算限制（Long Video / Streaming）：  
記憶體牆：受限於 Transformer 全局注意力的算力與顯存消耗，目前一次性 Forward 的極限約在 ~200 幀 左右。  
無法串流 SLAM：缺乏類似傳統 SLAM 的關鍵幀與滑動窗口/記憶庫機制，無法做到無限時間的「即時串流建圖」。  
11.名詞表  
Pointmap:大小為 H x W x 3 的矩陣，畫面中每個像素 (u, v) 都直接寫入一個 3D 空間座標 (X, Y, Z)  
SfM:SfM（Structure from Motion）是透過多張照片解算相機姿態與稀疏 3D 點雲的流程；BA（束網調整）則是 SfM 的最後一步，透過繁複的幾何公式最小化重投影誤差。  
3DGS:2023 年提出的場景表示法，用無數個三維高斯橢球體來代表世界，並透過可微柵格化達成極速渲染。  
Scale Ambiguity:單張 RGB 照片只含有角度與比例資訊，無法分辨眼前的是「距離 30 公分的玩具車」還是「距離 30 公尺的真車」。    
12.GFM vs 古典    
面向	     古典 3D (COLMAP)	             GFM 3D 大模型 (DUSt3R / VGGT)  
處理速度   慢（耗時幾分鐘～幾小時）	       極快（1～10 秒直出）  
拍攝要求	  嚴格（要紋理豐富、需相機校準）	   彈性（隨手拍、免校準、能過濾白牆）  
運作機制	  幾何公式推導（完全透明可解釋）	   神經網路推論（黑盒但具備空間常識）  
首選場景	  高精度離線測繪、文化遺產保存	     即時 SLAM、AR 空間理解、機器人感知  
## 學習記錄：(a) 你看到了什麼，查到了什麼，瞭解到了什麼，你又關心什麼具體現象 (b) 為什麼現有方法的問題是什麼？  
(a) 機器人與 3D 視覺徹底告別「古典分層工具鏈」、DUSt3R MASt3R 核心、VGGT技術、未來的研究機會  
DUSt3R, MASt3R, MASt3R-sfm, MUSt3R, PANSt3R影片:https://www.youtube.com/watch?v=AWu5z2O9H3s  
(b)10. 
